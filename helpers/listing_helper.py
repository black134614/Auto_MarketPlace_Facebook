...
def update_listings(listings, type, scraper):
    ...
    for listing in listings:
        ...
        # Publish the listing in the marketplace
        publish_listing(listing, type, scraper)

        # Add a delay of 5 minutes (300 seconds)
        timesleep = random.randint(1100, 1600)
...
def remove_listing(data, listing_type, scraper):
    ...
    scraper.element_click(
        'div[aria-label="Bài niêm yết của bạn"] div[aria-label="Xóa"]')

    # Click on confirm button to delete
    confirm_delete_selector = 'div[aria-label="Xóa bài niêm yết"] div[aria-label="Xóa"][tabindex="0"]'
    ...
    scraper.element_wait_to_be_invisible(
        'div[aria-label="Bài niêm yết của bạn"]')


def publish_listing(data, listing_type, scraper):
    # Click on create new listing button
    scraper.element_click(
        'div[aria-label="Thanh bên Marketplace"] a[aria-label="Tạo bài niêm yết mới"]')
    # Choose listing type
    scraper.element_click(
        'a[href="/marketplace/create/' + listing_type + '/"]')
    ...
    scraper.element_send_keys('label[aria-label="Giá"] input', data['Price'])
    scraper.element_send_keys(
        'label[aria-label="Mô tả"] textarea', data['Description'])
    scraper.element_send_keys(
        'label[aria-label="Vị trí"] input', data['Location'])
    scraper.element_click('ul[role="listbox"] li:first-child > div')

    next_button_selector = 'div [aria-label="Tiếp"] > div'
    ...
    # Publish the listing
    scraper.element_click('div[aria-label="Đăng"]:not([aria-disabled])')
...
def add_fields_for_item(data, scraper):
    scraper.element_send_keys(
        'label[aria-label="Tiêu đề"] input', data['Title'])

    # Scroll to "Category" select field
    scraper.scroll_to_element('label[aria-label="Hạng mục"]')
    # Expand category select
    scraper.element_click('label[aria-label="Hạng mục"]')
    # Select category
    scraper.element_click_by_xpath('//span[text()="' + data['Category'] + '"]')

    # Expand category select
    scraper.element_click('label[aria-label="Tình trạng"]')
    # Select category
    scraper.element_click_by_xpath(
    ...
    if data['Category'] == 'Sports & Outdoors':
        scraper.element_send_keys(
            'label[aria-label="Thương hiệu"] input', data['Brand'])
...
def post_listing_to_multiple_groups(data, listing_type, scraper):
    ...
    for group_name in group_names:
        ...
        scraper.element_click(
            '[aria-label="' + title + '"] + div [aria-label="Share"]')
...
def find_listing_by_title(title, scraper):
    searchInput = scraper.find_element(
        'input[placeholder="Tìm kiếm bài niêm yết"]', False)
    # Search input field is not existing
    if not searchInput:
        return False

    # Clear input field for searching listings before entering title
    scraper.element_delete_text('input[placeholder="Tìm kiếm bài niêm yết"]')
    # Enter the title of the listing in the input for search
    scraper.element_send_keys(
        'input[placeholder="Tìm kiếm bài niêm yết"]', title)
